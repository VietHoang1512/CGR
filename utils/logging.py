...
class TrainLogger:
    def __init__(self, args):
        ...
        self.best_val_wga, self.best_test_wga = 0.0, 0.0
        self.best_val_mean, self.best_test_mean = 0.0, 0.0
    ...
    def finalize_logging(self, model):
        ...
        save_checkpoint(
            model.state_dict(), join(self.args.output_dir, "final_checkpoint.pt")
        )
    ...
    def log_train_results_and_save_chkp(
        self, epoch, acc_groups, model, optimizer, scheduler
    ):
        if epoch % self.args.save_freq == 0:
            state_dict = create_state_dict(model, optimizer, scheduler, epoch)
            save_checkpoint(
                state_dict, join(self.args.output_dir, "resumable_checkpoint.pt")
            )
    ...
    def log_results_save_chkp(self, model, epoch, results_dict):
        ...
        if "val" in results_dict.keys():
            ...
            if val_wga > self.best_val_wga:
                ...
                save_checkpoint(
                    model.state_dict(), join(self.args.output_dir, "best_checkpoint.pt")
                )
        ...
        self.metrics[epoch].update(
            {"best_val_wga": self.best_val_wga, "best_test_wga": self.best_test_wga}
        )
        save_checkpoint(
            model.state_dict(), join(self.args.output_dir, f"checkpoint_{epoch}.pt")
        )
...
class EmbeddingsLogger(TrainLogger):
    ...
    def log_results_save_chkp(self, epoch, holdout_logits, model=None):
        ...
        test_acc_groups = self.generate_acc_groups(
            logits_test, self.test_y, self.test_groups, epoch, "test"
        )
        # logits_val = classifier(self.val_embeddings)
        val_acc_groups = self.generate_acc_groups(
            logits_val, self.val_y, self.val_groups, epoch, "val"
        )
        ...
        if val_wga > self.best_val_wga:
            ...
            if model is not None:
                save_checkpoint(
                    model.state_dict(),
                    join(self.args.base_model_dir, "best_last_layer.pt"),
                )
    ...
    def generate_acc_groups(self, logits, targets, groups, epoch, partition="train"):
        ...
        log_results(
            self.logger, self.metrics, epoch, acc_groups, partition, self.use_wandb
        )
        return acc_groups
